...
import { openModal, closeModal, showToast } from "/creations/shared/js/partner-shell.js";
import {
...
import { jobLogFailureDetail } from "./job-logs-fail-detail.js";

export { jobLogFailureDetail };

const FILTER_COLLAPSED_KEY = "admin_creations_logs_filter_collapsed";
const FILTER_TAB_KEY = "admin_creations_logs_filter_tab";
...
function statusHtml(row) {
  ...
  const label = escapeHtml(row.status_label || row.status);
  if (failed) {
    return `<div class="jl-row__status">
        <button type="button" class="jl-status jl-status--err is-clickable" data-jl-fail-info="${escapeHtml(row.job_key || "")}" aria-label="Show failure reason and restart">
          ${label}
        </button>
      </div>`;
  }
  return `<div class="jl-row__status">
        ${spin}
        <span class="jl-status ${statusClass(row.status)}">${label}</span>
      </div>`;
}
...
export async function mountJobLogs(container) {
  ...
  function openFailInfo(jobKey) {
    const row = (lastPayload.items || []).find((item) => item.job_key === jobKey);
    if (!row) return;
    const reason = jobLogFailureDetail(row);
    openModal({
      title: "Failed job",
      bodyHtml: `
        <p class="confirm-modal-message">${escapeHtml(row.title || row.product_title || "Untitled")}</p>
        <p><strong>Type:</strong> ${escapeHtml(row.type_label || row.type || "—")}
           · <strong>Source:</strong> ${escapeHtml(row.source_label || row.source || "—")}</p>
        <div class="field" style="margin-top:12px">
          <label>Failure reason</label>
          <div class="info-reason-box jl-fail-reason">${escapeHtml(reason)}</div>
        </div>
        <p class="jl-fail-hint">Restart re-queues the same process. Live Printify / Amazon products are not created again.</p>`,
      onSave: async () => {
        const result = await partnerFetch("admin-partner-job-log-resolve", {
          method: "POST",
          body: { job_key: jobKey, action: "retry" },
        });
        closeModal();
        if (result?.skipped) {
          showToast("Already live", result.message || "Nothing to restart");
        } else {
          showToast("Restarted", result.message || "Job queued again");
        }
    ...
    if (saveBtn) {
      saveBtn.textContent = "Restart process";
  ...
  listEl.addEventListener("click", async (event) => {
    const infoBtn = event.target.closest("[data-jl-fail-info]");
    if (infoBtn) {
      openFailInfo(infoBtn.getAttribute("data-jl-fail-info"));
      return;
    }
