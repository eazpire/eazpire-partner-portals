...
async function applyPartnerCatalogSchemaPatches(db) {
  ...
  try {
    const { ensureTodifyPartnerSetup } = await import("./partnerCatalog/todifyPartnerSeed.js");
    await ensureTodifyPartnerSetup(db);
  } catch (e) {
...
async function applyEazpireShadowSchemaPatches(db) {
  ...
  try {
    ...
    console.warn("[ensureManufacturerSchema] manufacturer_webhooks skipped:", e?.message || e);
  }
  try {
    await db
      .prepare(
    ...
    console.warn("[ensureManufacturerSchema] manufacturer_webhook_deliveries skipped:", e?.message || e);
  }
  try {
    await db
      .prepare(
