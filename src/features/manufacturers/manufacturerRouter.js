...
export async function handleManufacturerRouter(request, env, ctx) {
  ...
  if (ADMIN_OPS.has(op)) {
    ...
    if (op === "admin-partner-job-log-resolve" && request.method === "POST") {
      const { handleAdminPartnerJobLogResolve } = await import("../admin/adminJobLogs.js");
      return handleAdminPartnerJobLogResolve(request, env);
    }
    ...
    if (
      op === "admin-eazpire-test-printify-design-dimensions" &&
    ...
    if (op === "admin-creations-shopify-product-unpublish" && request.method === "POST") {
      const { handleAdminCreationsShopifyProductUnpublish } = await import(
    ...
    if (op === "admin-amazon-unpublish" && request.method === "POST") {
      ...
      return handleAdminAmazonUnpublish(request, env, ctx);
    }
