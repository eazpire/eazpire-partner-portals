...
export async function generatePreviewImage(env, { prompt, imageUrl }) {
  ...
  if (!env?.REPLICATE_API_TOKEN) {
    return { ok: false, error: "replicate_not_configured", message: "REPLICATE_API_TOKEN is not configured." };
  }

  try {
    const outputUrl = await runSeedream45(env, {
      prompt: text,
      image_input: [ref],
      size: "2K",
...
async function runSeedream45(env, input) {
  const waitSec = 55;
